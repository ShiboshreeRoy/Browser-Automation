...
def open_browser_tabs(urls, num_tabs, scroll_speed, scroll_duration, headless, random_clicks, num_clicks, proxy, close_tabs_after):
    try:
        options = Options()
        if headless:
        ...
        for i in range(num_tabs):
            ...
            if close_tabs_after > 0:
                time.sleep(close_tabs_after)
                driver.close()
...
def on_submit():
    try:
        ...
        num_clicks = int(entry_num_clicks.get())  # Fixed: Added entry_num_clicks reference
        proxy = entry_proxy.get().strip()
        close_tabs_after = int(entry_close_tabs.get())
        ...
        open_browser_tabs(urls, num_tabs, scroll_speed, scroll_duration, headless, random_clicks, num_clicks, proxy, close_tabs_after)
    except ValueError:
...
tk.Label(root, text="Close Tabs After (seconds):").grid(row=4, column=0, padx=10, pady=10, sticky="w")
entry_close_tabs = tk.Entry(root)
entry_close_tabs.grid(row=4, column=1, padx=10, pady=10)
entry_close_tabs.insert(0, "0")

tk.Label(root, text="Number of Random Clicks:").grid(row=5, column=0, padx=10, pady=10, sticky="w")  # Fixed: Added this line
entry_num_clicks = tk.Entry(root)  # Fixed: Added missing input field
entry_num_clicks.grid(row=5, column=1, padx=10, pady=10)
entry_num_clicks.insert(0, "3")

tk.Label(root, text="Proxy (optional):").grid(row=6, column=0, padx=10, pady=10, sticky="w")
entry_proxy = tk.Entry(root)
entry_proxy.grid(row=6, column=1, padx=10, pady=10)

headless_var = tk.BooleanVar()
tk.Checkbutton(root, text="Run in Headless Mode", variable=headless_var).grid(row=7, column=1, padx=10, pady=10, sticky="w")

random_clicks_var = tk.BooleanVar()
tk.Checkbutton(root, text="Enable Random Clicks", variable=random_clicks_var).grid(row=8, column=1, padx=10, pady=10, sticky="w")

submit_button = tk.Button(root, text="Start Automation", command=on_submit)
submit_button.grid(row=9, column=1, padx=10, pady=20)
